...
export function project(p,raw,profiles,platform={}){
 const db=Object.assign(blank(),structuredClone(raw));db.accounts=[];db.teachers=profiles.filter(x=>x.role==='teacher').map(profileRecord);db.students=profiles.filter(x=>x.role==='student').map(profileRecord);
 db.settings={...blank().settings,...raw.settings,...p.preferences?.settings};db.preferences={[p.role+':'+p.id]:p.preferences?.notifications||{}};
 if(p.role==='admin'){db.teachers=profiles.filter(x=>x.role==='teacher').map(profileRecord);db.students=profiles.filter(x=>x.role==='student').map(x=>({id:x.id,teacherId:x.teacher_id,status:x.data.status}));return db;}
 ...
 result.students=[student];result.teachers=db.teachers.map(t=>({id:t.id,name:t.name,coaching:t.coaching,status:t.status,features:t.features,storageLimit:t.storageLimit,expiry:t.expiry}));
...
const moduleFor={messages:'messages',materials:'materials',quizzes:'quizzes',attempts:'quizzes',payments:'fees',queue:'guardians'};
export function applyOperation(p,db,profiles,op,now=Date.now()){
 ...
 if(moduleFor[entity]&&p.role!=='admin'){
  ...
  requireThat(teacher?.data.features?.[moduleFor[entity]],'This module is disabled.');
 }
