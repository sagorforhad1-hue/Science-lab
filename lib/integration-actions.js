...
export async function integrationAction(c,action,b,{check,commit}){
 ...
 if(action==='ai-run'){
  ...
  let result,error;try{result=await generateAI(b.tool,b.prompt,b.language,aggregateContext(c),agent.instruction);entry.status='Completed';entry.tokens=Number.isInteger(result.usage)?result.usage:null;}catch(e){error=e;entry.status='Failed or unknown';}
