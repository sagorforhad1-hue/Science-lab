...
import {AI_TOOLS,SERVICE_DEFAULTS,providerStatus,permitted,serviceSettings,generateAI,sendEmail,sendPush,sendWhatsApp,firebaseConfig,firebaseValue,meetingURL,verifyConnections} from './providers.js';
...
export async function integrationAction(c,action,b,{check,commit}){
 ...
 if(action==='ai-limits'){
  ...
  const rows=check(await c.admin.from('sl_profiles').update({data:{...p.data,aiDailyLimit:b.daily,aiMonthlyLimit:b.monthly}}).eq('id',p.id).eq('data',JSON.stringify(p.data)).select('id'));
 ...
 if(action==='student-access'){
  ...
  const changed=check(await c.admin.from('sl_profiles').update({data:{...student.data,aiEnabled:b.enabled}}).eq('id',student.id).eq('data',JSON.stringify(student.data)).select('id'));requireThat(changed.length===1,'Account changed. Refresh and retry.',409);return {ok:true};
 }
 ...
 if(action==='ai-run'){
  ...
  let result,error;try{result=await generateAI(b.tool,b.prompt,b.language,aggregateContext(c));entry.status='Completed';entry.tokens=Number.isInteger(result.usage)?result.usage:null;}catch(e){error=e;entry.status='Failed or unknown';}
