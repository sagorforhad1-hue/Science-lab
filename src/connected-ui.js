...
async function openAssistant(tool){
 ...
 $('#ai-form').onsubmit=async ev=>{
  ...
  try{
   ...
   if(!$('#ai-result'))return;$('#ai-result').textContent=result.text;$('#ai-copy').hidden=false;$('#ai-copy').onclick=async()=>{try{await navigator.clipboard.writeText(result.text);toast(bilingual('Copied','কপি হয়েছে'));}catch{toast(bilingual('Select the draft and copy it.','খসড়া নির্বাচন করে কপি করো।'),true);}};
